...
import { GameConfigProps } from '../types.js';
...
function parseImageMap(image: ImageBitmap, callback) {
...
export default class Game {
  ...
  setup(assets) {
    ...
    const { dataMap: levelMap, canvas: miniMap } = parseImageMap(
      this.assets.level01,
    );

    this.level = new Level(
      this.assets.tileset,
      this.assets.tilesetMap,
      levelMap,
    );
